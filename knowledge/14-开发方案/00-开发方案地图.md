...
- [账号密码登录与数据库并发收敛](./0016-simple-login-and-database-hardening.md)（执行中，`active`；目标知识已更新，代码、迁移、机器契约和部署待实施）
- [第一阶段开发入口门禁](./0009-phase-1-entry-gate.md)（执行中，`active`；公网入口已完成，剩余真实手机、模型、证书续期和告警证据）
