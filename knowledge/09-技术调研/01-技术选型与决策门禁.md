...
status: "current"
summary: "技术建议只有在业务目标、候选证据、最小验证和用户确认完成后才能升级为当前架构决策。"
scope: ["architecture"]
...
- 上游边界：[产品范围与需求](../01-项目定义/03-产品范围与需求.md)、[系统上下文与总体边界](../03-系统架构/01-系统上下文与总体边界.md)
- 已接受选择：[决策内容地图](../10-架构决策/00-架构决策地图.md)
- 落实台账：[V2 首阶段技术基线](../14-开发方案/0004-v2-technology-baseline.md)
- 进入开发门禁：[第一阶段设计就绪验收](../11-测试与验收/02-phase-1-design-acceptance.md)
...
| 领域 | 选择 | 状态 | 决策记录 |
|---|---|---|---|
| 仓库 | 单仓库三套独立应用 | accepted | [决策 0004](../10-架构决策/0004-three-app-monorepo.md) |
| 手机 Web | React + TypeScript + Vite + React Router + TanStack Query | accepted | [决策 0006](../10-架构决策/0006-mobile-web-stack.md) |
| 手机样式 | Tailwind CSS + CSS 变量设计令牌 | accepted | [决策 0006](../10-架构决策/0006-mobile-web-stack.md) |
| 云端 API | Python 3.12 + FastAPI + Pydantic + Uvicorn | accepted | [决策 0005](../10-架构决策/0005-cloud-agent-runtime-stack.md) |
| 产品主 Agent | LangGraph；ArcMind 领域服务和数据库保持事实所有权 | accepted | [决策 0005](../10-架构决策/0005-cloud-agent-runtime-stack.md) |
| Python 工程 | uv + Ruff + Pyright + pytest | accepted | [决策 0007](../10-架构决策/0007-engineering-baseline.md) |
| TypeScript 工程 | Node.js LTS + pnpm workspace + ESLint + Vitest | accepted | [决策 0007](../10-架构决策/0007-engineering-baseline.md) |
| 端到端测试 | Playwright，覆盖 Chromium、WebKit 和 Firefox | accepted | [决策 0007](../10-架构决策/0007-engineering-baseline.md) |
| 公开契约 | OpenAPI 3.1 + JSON Schema；生成客户端类型 | accepted | [决策 0007](../10-架构决策/0007-engineering-baseline.md) |
| 本地与 VPS 编排 | Docker Compose | accepted | [决策 0007](../10-架构决策/0007-engineering-baseline.md) |
| 工作机 MVP | Electron + TypeScript + React/Vite 渲染层 | accepted | [决策 0007](../10-架构决策/0007-engineering-baseline.md) |
| 主数据库 | PostgreSQL 18 | accepted | [决策 0008](../10-架构决策/0008-primary-data-stack.md) |
| 数据访问与迁移 | SQLAlchemy 2.0 + psycopg 3 + Alembic | accepted | [决策 0008](../10-架构决策/0008-primary-data-stack.md) |
| 记忆向量索引 | 同库 pgvector，可重建且延迟启用 | accepted | [决策 0008](../10-架构决策/0008-primary-data-stack.md) |
| 耐久 Job 与提醒调度 | Procrastinate + PostgreSQL；领域提醒由数据库扫描生成触发实例 | accepted | [决策 0009](../10-架构决策/0009-durable-job-stack.md) |
| 身份实现 | 单一允许邮箱 + 6 位一次性验证码 + 服务端摘要会话 | accepted | [决策 0012](../10-架构决策/0012-personal-email-otp-identity.md) |
| 可信 HTTPS | 固定公网 IPv4 + Caddy + Let’s Encrypt 短期 IP 证书 | accepted | [决策 0013](../10-架构决策/0013-public-ip-https.md) |
| 文字事件通道 | 普通 HTTPS 控制请求 + SSE 服务端事件 | accepted | [决策 0014](../10-架构决策/0014-sse-text-event-channel.md) |
| 首个主 Agent 模型 | DeepSeek `deepseek-v4-pro`，经 ArcMind 适配端口接入 | accepted | [决策 0015](../10-架构决策/0015-first-model-provider.md) |
