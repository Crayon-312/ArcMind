...
- 上游尚未发布标准 Markdown 关系和状态优先检索时，ArcMind 使用包管理器的版本化补丁固定最小差异；项目版本标识为 `0.2.0-fd84369+arcmind-retrieval.1`，补丁不得复制为第二套运行层源码。
...
- 标准 Markdown 内部链接进入本地索引；默认检索优先 `current`，同时保留显式状态过滤和低权重直接邻接扩展。
