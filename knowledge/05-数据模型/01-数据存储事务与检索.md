...
status: "current"
summary: "当前仓库物理领域表为 users、auth_login_attempts、auth_sessions、conversations、turns、assistant_responses 和 response_events；未来目标表组不冒充已建表。"
scope: ["architecture"]
...
截至 2026-08-14，当前仓库迁移和生产实际使用的领域表如下；Procrastinate（PostgreSQL 后台任务队列）自有表由队列 Schema 独立管理：

| 模块 | 当前物理表 | 说明 |
|---|---|---|
| 身份 | `users`、`auth_login_attempts`、`auth_sessions` | 单一账号密码摘要、失败限速记录和服务端会话 |
| 对话 | `conversations`、`turns`、`assistant_responses`、`response_events` | 会话、最终轮次、响应状态和有序事件 |

当前物理表以 Alembic（数据库迁移工具）迁移、ORM（对象关系映射）模型和生产发布记录为实现证据。生产已升级到 `20260813_0003` 并移除 `auth_challenges`；`users.email` 只为[未绑定旧用户的数据归属确认](../13-已知问题/legacy-user-ownership-after-login-migration.md)保留，不参与公开登录。

## 目标表组与所有权

表名使用复数 `snake_case`。下表是未来完整产品能力的逻辑目标，不是当前已经创建的物理表，也不是最终 DDL（数据库定义语言）。

| 模块 | 主要表 | 阶段 | 说明 |
|---|---|---|---|
| 身份 | 独立身份、凭据和设备绑定关系 | 后续扩展 | 当前单账号实现先使用 `users`、`auth_login_attempts` 和 `auth_sessions`；多用户或多凭据需求确认后再拆表 |
| 对话 | 会话、轮次、响应与会话摘要 | 分阶段 | 当前使用 `conversations`、`turns`、`assistant_responses` 和 `response_events`；摘要表后续按实际需求创建 |
...
Procrastinate 自有表不纳入 ArcMind 领域表命名和 Repository 约定。其 `apply_schema` 只用于空库首次初始化，不能对已有结构重复执行；版本变化必须把供应商 SQL 迁移纳入已批准的发布任务。业务迁移不得自行改写组件内部表。
