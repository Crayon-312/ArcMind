...
截至 2026-08-13，当前仓库迁移实际创建并由运行时使用的领域表如下；Procrastinate（PostgreSQL 后台任务队列）自有表由队列 Schema 独立管理：
...
当前物理表以 Alembic（数据库迁移工具）迁移和 ORM（对象关系映射）模型为实现证据。生产在部署新版迁移前仍可能保留旧 `auth_challenges` 表，不能根据仓库结构推断线上已经迁移。
