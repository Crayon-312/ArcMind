...
status: "current"
summary: "内部 Job 使用至少一次投递和幂等消费，长工作机执行由租约跟踪而不长期占用云端 Worker。"
scope: ["architecture"]
...
- 数据基础：[数据存储、事务与检索设计](../05-数据模型/01-数据存储事务与检索.md)
- 业务状态：[任务生命周期](./02-任务生命周期.md)、[提醒与通知](./03-提醒与通知.md)
- 责任模块：[任务编排](./05-任务编排模块.md)、[提醒与通知模块（草案）](./04-提醒与通知模块.md)
- 技术决策：[耐久 Job 与提醒调度技术栈](../10-架构决策/0009-durable-job-stack.md)
