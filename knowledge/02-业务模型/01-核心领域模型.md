...
type: "business_rule"
status: "draft"
summary: "用户、会话、任务、步骤、执行尝试、记忆、提醒和设备是独立领域对象，具有不同所有权与生命周期。"
scope: ["domain"]
...
- 所属领域：[领域内容地图](./02-领域模型导航.md)
- 上游语言：[ArcMind V2 统一术语](../01-项目定义/01-统一术语.md)
- 上游需求：[产品范围与需求](../01-项目定义/03-产品范围与需求.md)
- 行为细化：[业务内容地图](./00-业务模型地图.md)、[模块内容地图](../03-系统架构/03-模块边界总览.md)
- 物理映射：[数据存储、事务与检索设计](../05-数据模型/01-数据存储事务与检索.md)

## 聚合边界

| 聚合 | 根对象 | 包含或关联 | 事实所有者 |
|---|---|---|---|
| 身份 | `User` | `LoginIdentity`、`CredentialRecord`、`UserSession`、`DeviceBinding`、偏好 | 身份与访问模块 |
...
```text
User
 ├─ 1 LoginIdentity ── 1 CredentialRecord
 ├─ 0..* UserSession
...
| 对象 | 必需语义字段 | 说明 |
|---|---|---|
| `User` | `id`、`status`、`locale`、`time_zone`、`created_at` | 登录账号由身份子模型单独保存 |
| `LoginIdentity` | `id`、`user_id`、`kind`、`normalized_value`、`status`、`created_at` | 首版 `kind` 仅允许 `username`，且部署只允许一个账号 |
| `CredentialRecord` | `identity_id`、`credential_digest`、`updated_at` | 只保存抗暴力哈希，不保存明文或可逆密文 |
...
本领域模型仍负责业务语义；已确认的 PostgreSQL 表组、字段约定、事务边界和检索设计见[数据存储、事务与检索设计](../05-数据模型/01-数据存储事务与检索.md)。领域对象、API DTO 和 SQLAlchemy ORM 模型必须保持分离，不能因为物理表已经确定就把本领域草案自动视为全部定稿。
