...
status: "current"
summary: "ArcMind 开发协作层固定使用 Agent Context OS 0.2.0 的 fd84369 上游提交，并以 knowledge Obsidian Vault 建立可重建本地索引。"
...
ArcMind 开发协作层以工作区开发依赖固定 Agent Context OS 0.2.0 上游提交 `fd84369ae9fd1d88df222d18a8519fba87592f4e`。`.agent-context/config.json` 只配置一个 `knowledge/` Obsidian 来源，本地索引写入被 Git 忽略的 `.agent-context/local-index/index.json`。升级当时的 122 条迁移结果属于历史发布证据；统一目录迁移后的当前数量以[最新知识结构发布记录](./2026-08-13-unified-knowledge-layout.md)和重建索引为准。
