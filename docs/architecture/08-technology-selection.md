...
公网 IP 证书已完成服务器签发、TLS（传输层安全协议）和普通客户端验证，当前隔离入口运行在公网 18443；但 IP 证书的 ACME 校验固定依赖公网 80 或 443，当前共享 VPS 约束下无法自动续期。生产必须另选域名 DNS 自动校验、ArcMind 独立服务器或可信隧道，并继续补齐真实手机的安全上下文、Cookie（浏览器会话凭据）和麦克风证据；这些剩余验收会继续阻止第一阶段总门禁通过。DeepSeek 首个模型方向已确认，真实质量和工具调用测试在获得部署密钥后完成。
