...
- 公网 HTTP 可达并返回 HTTPS 重定向，但公网 TCP `443` 从外部仍超时；主机监听、UFW、Docker 端口映射和内部 HTTPS 均正常，证据把剩余阻塞定位到腾讯云安全组。

因此，独立服务器已经解决共享主机端口冲突和协议上无法续期的问题，但公网入口尚未最终验收。必须在腾讯云控制台放行 TCP `443`，再从公网完成证书链、HTTP 200、登录、SSE 和真实手机复测；实际自动续期与低于 48 小时告警仍是独立门禁。
...
独立 VPS 仍需在腾讯云安全组放行公网 TCP `443` 后完成外部证书链、HTTP 200、登录和 SSE 验收；真实手机还需确认无证书警告、`isSecureContext` 为真、安全 Cookie 正常且麦克风权限可请求。实际自动续期和低于 48 小时告警尚未验证；完成前，DR-005 保持“部分通过”，不能宣布第一阶段总门禁完成。
