...
export function ParticleCore({ mode, signal, sidebarOpen = false, workbenchOpen = false, composerOpen = false }: ParticleCoreProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i += 1) {
      ...
      const radius = 1.08 + (band % 5) * 0.16 + Math.sin(shell * 0.023 + band) * 0.08
      const localX = Math.cos(angle) * radius
      const localY = Math.sin(angle) * radius * (0.9 + (band % 4) * 0.035)
      const localZ = Math.sin(shell * 0.071 + band * 0.6) * 0.1 + Math.sin(angle * 3.2 + band) * 0.035
    ...
    const ringGeometry = new THREE.TorusGeometry(2.08, 0.006, 10, 132, Math.PI * 1.46)
    ...
    const ringCount = Math.min(ringOrientations.length, Math.max(2, profile.ringCount * 2))
    const rings = ringOrientations.slice(0, ringCount).map((orientation, index) => {
    ...
    const animate = (): void => {
      ...
      const deltaSeconds = previousElapsed > 0 ? Math.max(0, elapsed - previousElapsed) : 1 / 60
      ...
      if (composerImpulseRef.current > 0) {
        ...
        composerProgress = clamp((composerImpulseAgeRef.current - COMPOSER_CONTACT_DELAY_SECONDS) / COMPOSER_IMPULSE_SECONDS, 0, 1)
        if (composerImpulseAgeRef.current >= COMPOSER_CONTACT_DELAY_SECONDS + COMPOSER_IMPULSE_SECONDS) {
          composerImpulseRef.current = 0
          composerImpulseAgeRef.current = 0
          host.classList.remove('is-composer-ripple-active')
        } else if (composerProgress > 0 && composerProgress < 1) {
          composerImpulse = Math.sin(composerProgress * Math.PI) * (1 - composerProgress * 0.12)
        }
      }
      sidebarInfluence += ((sidebarOpenRef.current ? 1 : 0) - sidebarInfluence) * 0.08
      workbenchInfluence += ((workbenchOpenRef.current ? 1 : 0) - workbenchInfluence) * 0.08
      ...
      for (let i = 0; i < particleCount; i += 1) {
        ...
        const composerBand = composerImpulse > 0 ? edgeWidth * Math.exp(-((edgeDistance - composerTravel) ** 2) / (0.32 * 0.32)) : 0
        const composerNearSource = composerImpulse > 0 ? edgeWidth * Math.exp(-(verticalTravel ** 2) / 0.16) : 0
        const composerSheet = composerImpulse > 0 ? 0.12 * Math.exp(-((verticalTravel - composerTravel * 0.72) ** 2) / 1.7) * edgeWidth : 0
        const composerBottomWeight = composerImpulse > 0 ? clamp(1 - (baseY + 1.95) / 3.2, 0, 1) : 0
        ...
        const x = baseX * pulse
        const y = baseY * pulse
        const composerPhase = Math.sin(edgeDistance * 7.2 - composerProgress * 11.5)
        const composerLift = (composerRipple * 0.46 + composerImpulse * composerSheet * 0.18) * (0.62 + composerBottomWeight * 0.52)
        const composerDepth =
          (composerRipple * 0.82 + composerSourceGlow * 0.38 + composerImpulse * composerSheet * 0.34) * (0.72 + composerBottomWeight)

        positions[ix] = x * cos - y * sin
        positions[ix + 1] = x * sin + y * cos + wave + composerLift * (0.76 + composerPhase * 0.18)
        positions[ix + 2] = baseZ * (1 + breath * 0.38 + audio.high * 0.12 + orbitalFlicker * 0.8) + Math.cos(elapsed + i * 0.013) * (breath + audioDrive) * 0.2 + composerDepth

        const targetRed = Math.min(1, modeColor.r + errorBurst * 0.55 + startupEnergy * 0.12)
        const targetGreen = Math.min(1, modeColor.g + tokenBurst * 0.18 + audio.mid * 0.08 + composerRipple * 0.14 + composerSourceGlow * 0.08)
        const targetBlue = Math.min(1, modeColor.b + tokenBurst * 0.16 + audio.high * 0.08 + composerRipple * 0.12 + composerSourceGlow * 0.08)
        const red = colorValues[ix] ?? 0
        const green = colorValues[ix + 1] ?? 0
        const blue = colorValues[ix + 2] ?? 0
        colorValues[ix] = red + (targetRed - red) * 0.014
        colorValues[ix + 1] = green + (targetGreen - green) * 0.014
        colorValues[ix + 2] = blue + (targetBlue - blue) * 0.014
      }
      ...
      const horizontalInfluence = sidebarInfluence * 0.74 - workbenchInfluence * 0.44
      ...
      camera.lookAt(sidebarInfluence * 0.28 - workbenchInfluence * 0.2, composerImpulse * 0.12, 0)
      core.rotation.x = elapsed * 0.44
      core.rotation.y = elapsed * 0.38
      core.scale.setScalar(1 + breath * 0.42 + audio.low * 0.22 + tokenBurst * 0.035 + startupEnergy * 0.18 + composerImpulse * COMPOSER_CORE_SCALE)
      coreMaterial.opacity = 0.34 + breath * 0.24 + tokenBurst * 0.06 + errorBurst * 0.18 + composerImpulse * 0.14
      coreMaterial.color.lerp(errorBurst > 0.12 ? MODE_COLORS.error : modeColor, 0.045)
      coreHalo.scale.setScalar(1 + audio.low * 0.5 + audio.rhythm * 0.28 + tokenBurst * 0.07 + startupEnergy * 0.34 + composerImpulse * COMPOSER_HALO_SCALE)
      haloMaterial.color.lerp(errorBurst > 0.12 ? MODE_COLORS.error : modeColor, 0.04)
      haloMaterial.opacity = profile.advancedGlow ? 0.07 + breath * 0.14 + audioDrive * 0.16 + tokenBurst * 0.035 + composerImpulse * 0.14 : 0.05 + breath * 0.08 + composerImpulse * 0.08
  ...
  return <div ref={hostRef} className="particle-core" data-core-shell="wrapped" aria-label="弦核 3D 视觉" />
}
